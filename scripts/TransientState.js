...
import { renderFacilityMinerals } from "./RenderFacilityMinerals.js" // Importing the function to render the facility minerals view
...
export const changeState = (type, eventValue) => {
    try {
        // If the type relates to an ID, make sure the value is parsed as an integer
        if (['governorId', 'facilityId', 'facilityMineralId'].includes(type)) {
            transientState[type] = parseInt(eventValue, 10) // Parse as integer with base 10
        } else {
            transientState[type] = eventValue // For other types, just assign the value directly
        }
    } catch (error) {
...
export const purchaseMinerals = async () => {
    try {
        ...
        const selectedGovernor = governors.find(item => item.id === parseInt(transientState.governorId))
        
        // Finding the selected facility mineral using the facilityMineralId from transientState
        const selectedFacilityMineral = facilityMinerals.find(item => item.id === parseInt(transientState.facilityMineralId))
